...
| `<workdir>/.library/intrinsic/` | `_install_intrinsic_manuals` :174 | Wipe-and-rewrite every boot. This is the **mount** half of the built-in tool plugin contract (`src/lingtai/tools/_plugin.py`): a tool package that ships a `plugin.json` declares which bundle directory holds its owned skill and which public capability name it installs under, and that manifest — discovered from disk, importing nothing — is what this method follows. `web_search` → `web` was a host-side special case and is now the manifest's own `manual.install_as`; `_LEGACY_MANUAL_DESTINATIONS` retains only `bash` → `shell`, for packages that ship no manifest. An unusable manifest is logged (`tool_plugin_manifest_invalid`) and skipped, never half-applied. |
