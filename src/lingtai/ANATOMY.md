...
| `<workdir>/.library/intrinsic/` | `_install_intrinsic_manuals` :174 | Wipe-and-rewrite every boot. Source is each tool package's `manual/` bundle, or — for a tool that is a real Agent Plugin (`tools/email/agent_plugin/`) — the Agent Skill its plugin owns, resolved by the filesystem-only `lingtai.tools._plugin.owned_skill_dir()`. The destination name is unchanged either way, so a tool's pinned `manual_path` does not move when it becomes a plugin. |
