...
| `adapters/browser_transport.py` | Production static HTTP(S) Adapter for the browser Core-owned `BrowserPort`; bounds DNS wait with one in-flight resolver job, pins vetted IPs, and preserves Host/SNI, selected lazily by browser setup. |
