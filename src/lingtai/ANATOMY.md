...
- **Curated MCP runtime pinning:** `_load_mcp_from_workdir` :795 looks up each init.json mcp entry's `source` in `mcp_registry.jsonl`; when it is exactly `"lingtai-curated"` (the imap/telegram/feishu/wechat/whatsapp/cloud_mail addons from `mcp_catalog.json`), the spawned `command` is overridden in-memory to this Agent's own `sys.executable`, and the child's `PYTHONPATH` gets this Agent's own currently-imported `lingtai` source root prepended (any existing configured `PYTHONPATH` on the entry is kept after it, deduplicated) — interpreter alone is not enough because the MCP stdio transport's default env allowlist excludes `PYTHONPATH`, so a curated child on the right interpreter could still resolve `lingtai` from that interpreter's own site-packages instead of this Agent's actual source. Every other env key and all args pass through unchanged; init.json and mcp_registry.jsonl are never rewritten. `_mcp_init_specs[name]["cfg"]` holds the post-override cfg, so `_retry_failed_mcps` respawns with the same pinned interpreter and source root. Non-curated (third-party/plugin/legacy `mcp/servers.json`) entries are unaffected — and daemon task/plugin MCPs, which spawn from their own task/plugin configs (never from `mcp_registry.jsonl`), are unaffected by this pin or by any registry reconcile.
