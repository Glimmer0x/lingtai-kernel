...
_LTP_V2_MIGRATED_FAMILIES = frozenset({"web", "soul"})
...
def summary_requested(args: dict | None, tool_name: str | None = None) -> bool:
    ...
    for every caller. A migrated LTP v2 family (currently ``web`` and ``soul``)
...
def maybe_summarize_result(
    ...
    *,
    ...
    # (``web``'s is exactly ``"failed"``, and every family built on the generic
    # ``ToolFamily`` dispatcher — including ``soul`` — returns ``"failed"`` for
    # its envelope-level errors); recognizing it here is scoped to migrated
    # families only, so an unrelated tool's non-error ``"failed"``-named domain
    # value is never reinterpreted as an error result.
