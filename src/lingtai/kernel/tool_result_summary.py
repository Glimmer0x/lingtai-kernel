...
_LTP_V2_MIGRATED_FAMILIES = frozenset({"web", "mcp", "knowledge"})
...
def summary_requested(args: dict | None, tool_name: str | None = None) -> bool:
    ...
    for every caller. A migrated LTP v2 family (currently ``web``, ``mcp``,
    and ``knowledge``) may instead set the canonical root ``summarize``
    boolean; that spelling is
