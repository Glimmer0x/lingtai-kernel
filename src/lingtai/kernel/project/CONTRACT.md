...
named agent with mailboxes, manifests, an inline caller covenant, and an
`init.json` based on the caller's preset. It does not start an Agent or create
...
caller covenant, uses wrapper `load_preset`, and injects the current init reader
as the adapter validator.
...
- The adapter creates the `.lingtai` directory exclusively, writes the complete
  seed, then verifies that the initial agent can be read by the current reader.
