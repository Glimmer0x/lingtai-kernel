...
- Read-only SQLite inspection opens ordinary URI `mode=ro` connections; `query()` accepts only `SELECT`/CTE/`EXPLAIN` statements and brackets each query with `PRAGMA query_only=ON`, while `doctor()` uses the same read-only connection. Query and doctor are read-only with respect to SQLite database contents and preserve the main database mtime, while ordinary live-safe `mode=ro` may create or update SQLite read-support `-wal`/`-shm` sidecar files as needed (`services/logging.py:208-225`, `services/logging.py:577-599`).
