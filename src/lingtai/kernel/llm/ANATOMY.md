...
- `session.py` resolves the create/rebuild thinking value via `SessionManager._session_thinking()` (`session.py:364-407`), which keeps the internal `THINKING_OMITTED` sentinel for routes that own their omission (Claude Code, Kimi Code, Codex, Zhipu/GLM) and applies the legacy `DEFAULT_THINKING` only for a genuinely constructor-omitted value on every other provider. The optional `llm_call.reasoning` record is added by the `llm_call` observer, which reads an adapter-provided immutable capture — `reasoning_construction_result()` (`llm/base.py:221-223`) and the allowlisted `reasoning_observability()` accessor (`session.py:94-107`, `session.py:520-533`).
