...
An intent already active at ordinary `lingtai-agent run` is the **early
refusal**: it returns mechanical code `explicit_suspend_active` before Agent,
provider, MCP, or ToolPlugin construction, and an existing `.suspend` marker is
preserved. A suspend arriving after that read is the separate **late race**:
the Agent may already be constructed and ordinary stale-marker cleanup may
already have occurred, but locked boot registration re-reads durable intent and
refuses its append. The constructed-but-unstarted Agent is stopped and no boot
row/start occurs. A System suspend marker written after cleanup remains truthful;
regardless of marker timing, durable intent is authoritative. After matching
CPR, ordinary stale `.suspend`/`.sleep`/`.refresh` cleanup and boot registration
proceed before start. The reader does not repair or ignore corrupt/unsupported/
torn or impossible boot-after-suspend history: it exits nonzero.
...
tracebacks. New v2 mechanical codes include `ledger_agent_dir_unavailable`,
