...
def _cli_reasoning_construction(
    ...
    frozen ``KimiEffort`` carrier (its env fragment) owns the
    ``KIMI_MODEL_THINKING_EFFORT`` variable on the actual command. This
    mapper therefore always contributes an empty argv/env delta and records
    ``omitted`` — it never claims a configured value was dropped, because
    the value reaches the CLI through the effort carrier. The command
    builder (``_invoke_raw``) consumes this result for argv/env deltas, so
    a future CLI reasoning flag must flow through here or the record goes
