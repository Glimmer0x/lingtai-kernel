...
maintenance: |
  ...
  `src/lingtai/mcp_catalog.json` in related_files with it. `mcp` and `avatar`
...
- **Registry families** — the intrinsics and built-in capability rows
  registered through `src/lingtai/tools/registry.py`. `mcp`, `avatar`, and
  `plugin` are first-party families and are in scope *as families*; the external
  records they render are not. `mcp` and `avatar` are declared under the
...
any family beyond `mcp` and `avatar`; any family registered through
