...
# Job-Object active-process confirmation can take its full 5s, and the
# identity-gated taskkill fallback sweep plus bounded root reap adds up to
# ~12s worst case.  A tighter 3s window flaked the native Windows cancel
# contract under runner load (manager gave up with "awaiting supervisor
# terminal commit" while the supervisor was still committing).
_CANCEL_COMMIT_TIMEOUT_SECONDS = 15.0
