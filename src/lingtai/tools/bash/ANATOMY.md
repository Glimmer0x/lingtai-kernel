...
- `bash/__init__.py` — the execution engine plus policy, sync execution, and durable async manager orchestration. It declares no schema or description of its own: the package's single public pair is re-exported from `_tool_family.py` at `__init__.py:56` under the canonical `get_schema`/`get_description` names, so there is exactly one model-facing surface and no second flat one to drift against. `setup` (`__init__.py:2063`) registers the public tool under `SHELL_PLUGIN.name` — not a literal — with that schema/description and the family dispatcher, so the registered tool, the composed family, and the installed-manual destination are one name. `ShellManager` (`__init__.py:570`, aliased `BashManager` at `__init__.py:2060`) owns validation, manager semantics, durable-state rehydration, notification publication, and poll/cancel consumption; its `handle` dispatches `poll`/`cancel`/`run` only — `manual` is a family child and never reaches it. Its async start receipt and durable reminder body both teach the registered envelope call `shell(action="poll", input={"job_id": ...})`. `_augment_command_result` adds `ok`/`command_status`/`warning` fidelity fields (`__init__.py:424`). Sync `_run_sync` feeds a dialect's `stdin_script` through `subprocess.run(input=...)` so the PowerShell bootstrap receives the real command over UTF-8 stdin.
- `bash/_tool_family.py` — the public/model-facing `shell` tool's LTP v2 envelope. `RUN_INPUT_SCHEMA`/`POLL_INPUT_SCHEMA`/`CANCEL_INPUT_SCHEMA`/`MANUAL_INPUT_SCHEMA` are four strict per-action schemas (run-only fields — `command`, `timeout`, `working_dir`, `async`, `reminder` — live only in `run`'s branch; `job_id` lives only in `poll`/`cancel`'s); `get_schema()` composes them via the generic `tool_family.ToolFamily` (root `allOf` correlation plus `input.oneOf` disclosure, same infra `web` uses), and `get_description()` here is the registered model-facing text documenting that action-separated call shape. Optional run fields are declared required-but-nullable per the strict-schema convention; `_strip_nulls` drops `null` (meaning *absent*) so `ShellManager`'s own runtime defaults apply, while falsy-but-present values (`timeout: 0`, `async: false`) pass through verbatim. `ShellFamilyDispatcher` is the per-agent, per-`ShellManager` handler registered by `setup()`: its `run`/`poll`/`cancel` child handlers flatten their own validated `input` (re-adding the matching `action` key) and delegate to the unchanged `ShellManager.handle` — the async lifecycle, durable state, and every existing `ShellManager` test keep exercising that exact code path. Its thin outer `handle()` adds exactly one Host normalization: narrowing the generic dispatcher's unknown-action message to Shell's own four actions. This module declares only Shell's *own* three branches (`_DECLARED_INPUT_SCHEMAS`, asserted equal to `SHELL_DECLARED_ACTIONS`): both the module-level schema-only family and the per-agent dispatcher are composed by `SHELL_PLUGIN.build_family`, which appends the reserved `manual` child bound to the packaged `manual/SKILL.md`, so this module cannot omit, re-schema, or rebind it. `MANUAL_INPUT_SCHEMA` comes from `SHELL_PLUGIN.manual_input_schema()` (a deep copy of the one shared `tool_family.manual` literal) rather than being re-declared, so the schema the wire advertises and the schema dispatch validates against are one definition, byte-identical to every other family's. `_ACTIONS_PROSE` derives the unknown-action message from the plugin-composed action list, so a new action cannot leave it behind. The engine has no manual branch at all — `manual` performs no shell operation and never reaches `ShellManager`.
- `bash/plugin.py` — the package's **plugin descriptor**, the one place Shell states who it is: public capability/family name (`shell`), retained implementation module (`lingtai.tools.bash`), packaged skill (`shell-manual`), core-default kwargs (`{"yolo": true}`), its own action list (`SHELL_DECLARED_ACTIONS = run | poll | cancel`, with `manual` deliberately absent), and the plugin-composed public list `SHELL_ACTIONS`. `SHELL_PLUGIN` is a `lingtai.tools._plugin.KernelToolPlugin`; it loads and name-checks `manual/SKILL.md` at import, so a renamed or lost manual fails loudly instead of serving foreign or empty material. `_tool_family.py` consumes it for the schema and dispatch, `__init__.py` for the registered tool name, `registry.py`'s `BUILTIN_TOOLS`/`CORE_DEFAULTS` entries must equal `capability_declaration()` (proven by `tests/test_shell_tool_plugin_package.py`, not by importing the package into the registry), and `Agent._install_intrinsic_manuals` reaches it through `_plugin.manual_destination_for("bash")` to learn the `bash/manual/` → `capabilities/shell/` mapping it used to spell as a literal. Declarative only: it holds no policy, dialect, containment, or async machinery — `ShellPolicy`, `ShellDialect`, the sandbox check, and the durable supervisor are unchanged and stay behind `setup()`/`ShellManager` (`src/lingtai/tools/_plugin.py`).
