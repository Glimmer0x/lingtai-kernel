...
- `bash/_descriptor.py` — the package-local source for the canonical model-facing name `shell`, ordered `run | poll | cancel | manual` action ports, and installed-manual destination `shell`. The strict family schema, dispatcher, manual child, public description, and package setup bridge consume it; it owns no global registry, Agent guard, policy, or process behavior.
