...
the fail-fast registrar. `src/lingtai/tools/mcp/__init__.py` `DECLARATION` is the first declared slice
— `mcp` binds against two granted host ports instead of the whole `Agent`.
`src/lingtai/tools/context/__init__.py` is the second: Context keeps its
mandatory intrinsic hook/transport shim but the official mount alone publishes
its model-facing surface, binding only `workdir` and `context_runtime`; the
latter preserves live molt/summarize/rebuild semantics through three narrow
callbacks. Both retain their public names, actions, inputs, and results. Every
