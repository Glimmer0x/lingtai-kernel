...
static `ToolPluginDeclaration`, earned least-privilege ports, the reserved
`OFFICIAL_TOOL_PLUGIN_NAMES` list, and the fail-fast registrar.
`src/lingtai/tools/mcp/__init__.py` is the presentation slice: `mcp` binds
against `workdir` and `prompt_section`. `src/lingtai/tools/daemon/__init__.py`
is the manager-owning slice: `daemon` binds against `workdir` and its named
`daemon_runtime` port, preserving its current model, tool-surface, preset,
notification, time, Task Card, and logging semantics without taking an Agent.
Both keep their public names, actions, schemas, manuals, and result shapes;
