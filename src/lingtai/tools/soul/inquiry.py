...
def soul_inquiry(runtime: "SoulRuntimePort", question: str) -> dict | None:
    ...
    if runtime.chat is not None:
        for entry in runtime.chat.interface.entries:
    ...
    system_prompt = _build_soul_system_prompt(runtime)
    ...
    try:
        ...
        runtime.log("soul_whisper_error", error=str(e)[:200])
        return None

    response = _send_with_timeout(runtime, session, question)
...
def _publish_human_inquiry_notification(runtime: "SoulRuntimePort", result: dict, question: str) -> None:
    ...
    runtime.log("btw_notification_published", question=question[:200])
    try:
        runtime.wake_nap("btw_inquiry_published")
    except Exception as e:
        runtime.log("btw_notification_wake_error", error=str(e)[:200])


def _run_inquiry(runtime: "SoulRuntimePort", question: str, source: str = "agent") -> None:
    """Run soul.inquiry and log result as insight event."""
    from .flow import _persist_soul_entry

    try:
        result = soul_inquiry(runtime, question)
        if result:
            runtime.log(
                "insight", text=result["voice"], question=question, source=source
            )
            _persist_soul_entry(runtime, result, mode="inquiry", source=source)
            if source == "human":
                _publish_human_inquiry_notification(runtime, result, question)
        else:
            runtime.log("insight", text="(silence)", question=question, source=source)
    except Exception as e:
        runtime.log("insight_error", error=str(e)[:200], question=question)
