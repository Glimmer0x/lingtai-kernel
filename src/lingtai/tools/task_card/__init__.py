...
class TaskCardManager:
    ...
    def _start_child(self, input_: dict[str, Any]) -> dict[str, Any]:
        ...
        interval_s = self._coerce_positive(
            input_.get("interval_s", _DEFAULT_INTERVAL_S), "interval_s", _MIN_INTERVAL_S
        )
        timeout_s = self._coerce_positive(
            input_.get("timeout_s", _DEFAULT_TIMEOUT_S), "timeout_s", 0.1
        )
        requested_max = input_.get("max_refreshes")
        if requested_max is None:
            effective_max = _DEFAULT_MAX_REFRESHES
        elif type(requested_max) is int and requested_max > 0:
            effective_max = min(requested_max, _DEFAULT_MAX_REFRESHES)
        else:
