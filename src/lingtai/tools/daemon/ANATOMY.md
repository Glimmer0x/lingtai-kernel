...
- **No recursion:** generic emanations retain `EMANATION_BLACKLIST`, which prevents sub-emanations, avatars, psyche, the skill catalog, or codex-style recursive agent execution. A persistently restricted Driver-derived child exposes only daemon/avatar from that set; each real nested request reaches Driver, receives a typed denied decision with an audit id, and remains structurally rejected by Core before nested provider I/O. The real supervisor/execution-child handoff and these daemon/avatar dispatches are covered by the 2b lifecycle probes; the candidate is ready for independent review, not direct merge approval. The universal detached admission requirement is a separate fail-closed fact; the surface decision reads only the durable v3 manifest's `derived_launch_admission_required`, never a transient launcher environment.
...
- **Detached daemon supervisor (unconditional all-backend route).** `_handle_emanate` validates and writes a secret-free manifest, then the POSIX adapter launches one detached supervisor per run. `execution_host.py` composes the existing `DaemonManager`/`_BackendSpec` setup and runners in that process, so LingTai, Claude, Codex, OpenCode, MiMo, Qwen, Oh-My-Pi, Kimi, and Cursor retain one production parser each. The supervisor owns deadline/control, exact child process groups, run-owned diagnostics, terminal `DaemonRunDir` truth, and notification publication; the parent manager retains only submit/inspect/control state. Presets, selected skills, task MCP registrations, and the built-in `daemon_common.checkpoint` / `daemon_common.finish` surface are reconstructed from the validated manifest and run directory. A constrained Driver-derived run transfers one opaque child endpoint from root to supervisor and then to the exact execution child; it is neither persisted nor exposed as an fd to Core, and setup failure deterministically closes it. The persistent manifest marker means only that authority is required; it is never a bearer or grant. Resolved credentials are never written to the manifest; request/control files and supervisor logs use restrictive permissions where supported. `system.refresh`/agent stop only shuts down parent-local resources and cannot see detached futures or child handles.
