...
- **Startup retries unpublished terminal notifications:** after stale-parent reaping, `DaemonManager.__init__` scans terminal run dirs and republishes only `done`/`failed`/`cancelled`/`timeout` records that explicitly carry `terminal_notified=false`. A stale `terminal_notification_claim` on a new-schema record is retryable; legacy records with `terminal_notified=true` or with the key absent are treated conservatively as already handled. The retry uses `daemon-terminal:<run_id>` as the event idempotency key, deduping a publish-before-receipt crash while same-run events remain in the per-run mini-channel without a fixed 20-event cap; after an intentional dismissal, recovery may safely republish because the durable receipt was never falsely recorded.
...
- **List is an index, not a second truth:** `daemon(action="list")` rebuilds its view from the active registry and each run directory's `daemon.json`/`.prompt`/result preview, with optional `contains`/`status`/`last` filtering. Omitted/null `last` defaults to the newest 1000 rows at the manager's record-row/materialization path; an explicit positive `last: N` (including N > 1000) overrides it. The current directory layout still requires reading candidate `daemon.json` records to preserve sorting/filtering. For current-version records, bounded no-`contains` list reads materialize prompt/result entry details only for rows that can be returned. A non-empty `contains` preserves full candidate materialization because it searches prompt-preview text before filtering and paging; the deferred performance benefit applies when `contains` is absent or empty. Missing, corrupt, or stale records still take the candidate-wide migration/rebuild path required to repair the index. If `daemon.json` is missing, corrupt, or has a stale `data_version`, list writes a best-effort rebuilt JSON and includes its `migration` reason in the index. Full detail stays in `.prompt`, `result.txt`, and JSONL trace files.
