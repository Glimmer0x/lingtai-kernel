...
| Every denied derived daemon launch preserves `reason_code` and `audit_id` in its public result through the shared error-result mapper; the AST guard covers direct and MRO-base handlers (`PermissionError`, `OSError`, `Exception`, `BaseException`) | `src/lingtai/tools/daemon/__init__.py` | `tests/test_cli.py::test_persisted_derived_child_exposes_both_tools_and_denies_nested_launches`, `tests/test_daemon.py::test_profile_external_cli_daemon_launch_reaches_admission_before_supervisor`, `::test_daemon_error_boundaries_share_admission_evidence_mapper` |
