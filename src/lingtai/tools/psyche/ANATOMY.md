...
- `__init__.py` — Package surface and the LTP v2 family composition. Re-exports the sub-module API. Contains:
  - The eight canonical child `input` schemas, one per migrated operation (`__init__.py:80-181`) — each action's own strict, closed object, declared exactly once.
  - `_CHILD_SPECS` / `ACTION_ORDER` (`__init__.py:200-212`) — the single registry of (name, schema, handler) plus the derived public action order. It replaces the former `_VALID_ACTIONS`/`_DISPATCH` pair: because the model-facing schema and the dispatch allow-list are generated from this one source, a child can never be schema-advertised but dispatch-rejected.
  - `_MOLT_ENVELOPE_KEYS` / `_strip_nulls` / `_build_children` (`__init__.py:223-275`) — the out-of-band envelope metadata psyche threads to `context_molt`, the null→absent normalizer that preserves pre-migration absent-vs-null handler behavior, and the builder that binds all nine children (the reserved `manual` from `build_manual_child` appended last).
  - `_FAMILY` (`__init__.py:278`) — the module-level schema-only `ToolFamily`; constructing it at import time is also the registry's duplicate/reserved-name collision check.
  - `get_description` / `get_schema` (`__init__.py:307-327`) — tool registration; `get_schema` returns the `ToolFamily`-composed closed envelope (root `action`/`input`/`reasoning`/`summarize` plus the `allOf` action/input correlation) with psyche's own action-routing prose substituted for the generic composer's placeholder.
  - `_adapt_manual_result` (`__init__.py:330-348`) — Host-owned, post-dispatch flattening of the reserved `manual` child's canonical result back to psyche's pinned flat shape.
  - `handle()` (`__init__.py:351-395`) — the family root: lifts `_tc_id`/`reasoning`/`_initiator` out of the closed root, builds the agent-bound `ToolFamily` per call, and normalizes the generic unknown-action envelope to psyche's own error.
  - `boot()` (`__init__.py:398-407`) — boot-time hook: loads lingtai + pad, registers post-molt reload callback.
...
- The former two-key `(object, action)` matrix is one flat action enum; each pre-migration pair maps to exactly one action (`pad`→`edit` is `pad_edit`, and so on). The flat pre-migration call shape is an unknown action, not a compatibility alias.
