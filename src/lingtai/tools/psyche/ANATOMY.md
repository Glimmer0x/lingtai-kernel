...
- `__init__.py` — Package surface. Preserves existing Python/module imports by re-exporting the public handler API; this is not a compatibility alias for the former public wire shape. Contains:
  - `get_description` / `get_schema` — tool registration; canonical contract — a closed `action` enum (the former `(object, action)` matrix folded into compound names, e.g. `pad_edit`, since `load` alone collides between `lingtai` and `pad`) plus a strict per-action `input` `anyOf`, each branch `additionalProperties: false`. `reasoning` is never declared here — `BaseAgent._build_tool_schemas` injects it kernel-wide.
  - `_ACTION_INPUT_FIELDS` / `_DISPATCH` — flat action→closed-input-field-set and action→handler tables; `_ROOT_TOLERATED` is the closed root-key set (`action`, `input`, `reasoning`, `_reasoning`, `_tc_id`) accepted alongside them.
  - `_current_setting()` — builds the fresh, copy-safe, no-op `current_setting` snapshot at call start, before validation or handler dispatch (via the shared `lingtai.tools._settings` reader).
  - `handle()` — main dispatcher: captures settings first; validates root/input object and string-key shape, the closed root set, action enum, required/unsupported input fields, and consequential value types — all before any handler runs — then unwraps `input` (dropping `None`-valued semantic optionals, since strict schemas express "absent" as `null`) into the SAME flat-arg shape the pre-migration handlers received. It copies handler/manual mappings before attaching a copy of the call-start `current_setting`, so shared handler results cannot be mutated.
  - `boot()` — boot-time hook: loads lingtai + pad, registers post-molt reload callback.
...
- `_molt.py` — Context molt core, name handlers, and system-initiated molt.
  ...
  - `context_forget()` (`_molt.py`) — system-initiated forced molt: bypasses public schema dispatch and synthesizes an internal ToolCallBlock+ToolResultBlock replay pair using canonical action/input key names; its input carries `summary`, null `session_journal_path`, null `keep_tool_calls`, and the actual `keep_last`. It then wipes context, replays the pair into the fresh interface, and persists the system-authored summary. Called by `base_agent/lifecycle.py` when an external `.clear` signal arrives; tests also exercise `source="warning_ladder"` and `source="aed"` directly. Same notification tracking reset as `_context_molt` (wire-level only; `.notification/` files survive). Also publishes a `post-molt` notification (`initiator: "system"`, `source` propagated) so the fresh agent has the same recall hook agent-initiated molts get.
