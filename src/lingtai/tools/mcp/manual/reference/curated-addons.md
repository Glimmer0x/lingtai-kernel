...
- **A venv swap does not move live children by itself, and boot/refresh never rewrites existing registry records.** When the runtime venv moves (latest-main owner rollout), the rollout procedure must **explicitly and manually** reconcile the curated record's `command` after the authorized venv swap to keep the durable registry (`mcp_registry.jsonl` on disk — not `mcp info`, which never surfaces `command`/`args`) truthful for anyone inspecting the file directly. There is no kernel helper for this: `decompress_addons` stays append-only/non-destructive to existing records and the `mcp` capability never writes the registry. The rollout performs the reconcile itself as a manual step using ordinary `write`/`edit`/`bash` — the self-contained command in the main SKILL.md "Runtime venv swap" section is the recipe. Its contract is explicit: it switches **every canonical curated record** to the new interpreter (no command-only override is preserved), it runs canonical `read_registry()` validation fail-closed first and **aborts without writing** if the registry has any invalid/duplicate/problem line, and it only rewrites the canonical first-valid row per name. This keeps durable registration consistent — but the registry record is purely a registration/gate artifact, never a child's spawn source, for any route. For a non-curated or legacy child, the live interpreter/env still come from that entry's own activation spec (`init.json`'s `mcp.<name>` or `mcp/servers.json`). For a **daemon-launched** MCP (task `mcp` registrations, plugin `mcp.json` servers), the live command/env come from that task/plugin's own config, never from this registry, so this reconcile has no effect on a daemon-launched child either way — only a curated **main-agent** `init.json` entry is affected, and even there the effect is cosmetic to the registry file only: the Agent derives its live `type`/`command`/`args`/`PYTHONPATH` fresh from its own currently-imported `mcp_catalog.json` and its own `sys.executable`/source root, in memory at load time, ignoring both whatever `init.json` stores and this registry record entirely — so this reconcile is never required for that child's own spawn, only for keeping the durable registry record itself truthful. Either way, `system(action="refresh")` only retries failed MCPs; it never restarts healthy children, so a full agent relaunch is required before an already-running curated child picks up a new venv — it then re-derives its launcher from the new interpreter's own catalog and source root on that relaunch. Verify with a fail-closed provenance gate (the one-shot probe above): if the probe still resolves to the old venv, the gate FAILS and the next step is a relaunch — do not claim PASS.
