...
2. **Add the addon to `init.json`.** Append the registry name to the top-level `addons:` list, then add an `mcp.<name>` activation entry with the subprocess spec from this table or the addon docs. A stdio activation requires a `command`, but for a curated addon its actual value — and the child's `PYTHONPATH` — are overridden at load time to the running Agent's own interpreter and source root — see "MCP child processes run in their own runtime" below — so neither needs manual upkeep across a venv move:
...
Each current curated addon shown in this file is launched as a separate **stdio Python subprocess** with its own environment. `args` and `env` always come from the **active activation spec** — `init.json`'s `mcp.<name>` entry, or `mcp/servers.json` for the legacy direct route. The **interpreter and import source** depend on the route: a curated addon activated via `init.json`'s `mcp.<name>` entry (registry record's `source` exactly `lingtai-curated`) is pinned in memory to the running Agent's own `sys.executable`, and its child `PYTHONPATH` gets this Agent's own currently-imported `lingtai` source root prepended (any `PYTHONPATH` already configured on the entry is kept after it, deduplicated) — regardless of what `command`/`PYTHONPATH` `init.json` stores. Interpreter alone would not be enough: the MCP stdio transport's default env allowlist excludes `PYTHONPATH`, so a curated child spawned on the right interpreter could still resolve `lingtai` from that interpreter's own site-packages instead of this Agent's actual source; the `PYTHONPATH` prepend is what makes the child's package source actually match the main agent's. Neither `init.json` nor `mcp_registry.jsonl` is rewritten by this pin. Every other route — the legacy `mcp/servers.json` direct mount, or a non-curated/third-party `init.json` entry — still launches with the `command`/`env` named in its own activation spec, not from the registry record and not pinned to the main agent. Daemon-launched MCPs (task `mcp` registrations, plugin `mcp.json` servers) are a separate route entirely: they spawn from that task/plugin's own config, never from `init.json`'s `mcp.<name>` entries or `mcp_registry.jsonl`, so this pin does not apply to them. Do not assume a non-curated, legacy, or daemon-launched child shares the main agent's Python, site-packages, `PYTHONPATH`, or environment:
...
- **Behavior that lives in an MCP child is only as new as that child's code.** Rendering, projection, and channel-side logic inside `lingtai.mcp_servers.*` (for example Telegram Task Card footer projection) executes in the child. After a code change to those modules, verify the child's interpreter and import resolution before reporting that a change is live: run a one-shot probe with the exact configured `command` and `env` that prints `sys.executable`, `lingtai.__file__`, and the selected `lingtai.mcp_servers.<name>.__file__`. For a curated addon, the interpreter half of that probe is the running Agent's own `sys.executable` and the import-resolution half is governed by the prepended `PYTHONPATH`, neither necessarily the `command`/`PYTHONPATH` strings stored in `init.json`. (Process inspection such as `lsof` is at best a platform-specific corroboration, not proof of the loaded Python module.)
- **The child venv is a normal package environment.** Site-packages for that interpreter is authoritative unless the activation `env` (or, for a curated addon, the pinned `PYTHONPATH`) overrides the import path. For a curated addon, that interpreter and source root are the main agent's own, so diagnosing "change not visible" starts with whether the Agent process itself has relaunched onto the new venv. For a non-curated, legacy, or daemon-launched child, it still starts with that entry's own activation `command`/`env` tuple. HTTP MCP entries have no local subprocess or interpreter at all; the stdio/subprocess guidance above applies only to stdio entries.

## Venv swap does not auto-reconcile registry records

- **A venv swap does not move live children by itself, and boot/refresh never rewrites existing registry records.** When the runtime venv moves (latest-main owner rollout), the rollout procedure must **explicitly and manually** reconcile the curated record's `command` after the authorized venv swap to keep the durable registry (`mcp info`, the `<registered_mcp>` prompt, manual copies) truthful. There is no kernel helper for this: `decompress_addons` stays append-only/non-destructive to existing records and the `mcp` capability never writes the registry. The rollout performs the reconcile itself as a manual step using ordinary `write`/`edit`/`bash` — the self-contained command in the main SKILL.md "Runtime venv swap" section is the recipe. Its contract is explicit: it switches **every canonical curated record** to the new interpreter (no command-only override is preserved), it runs canonical `read_registry()` validation fail-closed first and **aborts without writing** if the registry has any invalid/duplicate/problem line, and it only rewrites the canonical first-valid row per name. This keeps durable registration consistent — but the registry record is a registration/gate artifact, never a child's spawn source, for any route. For a non-curated or legacy child, the live interpreter/env still come from that entry's own activation spec (`init.json`'s `mcp.<name>` or `mcp/servers.json`). For a **daemon-launched** MCP (task `mcp` registrations, plugin `mcp.json` servers), the live command/env come from that task/plugin's own config, never from this registry, so this reconcile has no effect on a daemon-launched child either way — only a curated **main-agent** `init.json` entry is affected: the Agent pins its live `command` to its own `sys.executable`, and prepends its own currently-imported `lingtai` source root to the child's `PYTHONPATH`, in memory at load time, ignoring whatever `init.json` stores — so this reconcile is not required for that child's own spawn, only for keeping the durable registry record itself truthful. Either way, `system(action="refresh")` only retries failed MCPs; it never restarts healthy children, so a full agent relaunch is required before an already-running curated child picks up a new venv — it then self-pins to the new interpreter and source root on that relaunch. Verify with a fail-closed provenance gate (the one-shot probe above): if the probe still resolves to the old venv, the gate FAILS and the next step is a relaunch — do not claim PASS.
