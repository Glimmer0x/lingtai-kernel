...
under the kernel-owned declared host-plugin contract; the shared-C target also
names `email`, `file`, `context`, `notification`, `soul`, `vision`, `web`,
`daemon`, `system`, and `task_card` without claiming those candidate slices have
merged. Avatar is separately landed vertical evidence with its own declaration
and earned `avatar_parent` port, rather than evidence that any C target merged
(`src/lingtai/kernel/tool_plugin/ANATOMY.md`,
