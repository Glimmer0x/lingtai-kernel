...
### Model-facing action/input boundary

`get_schema()` returns the raw module schema and is the single canonical schema
object. It is a closed root object requiring `action` and `input`; `input.anyOf`
has exactly two closed, empty object branches titled `info input` and
`manual input`. No `reasoning` property is declared by this module. During
`BaseAgent._build_tool_schemas()`, BaseAgent alone adds optional root-level
`reasoning`; it does not alter either input branch. `ToolExecutor` removes public
`reasoning` before dispatch and carries it as internal `_reasoning` metadata.

The handler's one normalization boundary reads `read_settings(agent, "mcp")`
and `current_setting(snapshot, "mcp")` first on every call, accepts only
`action`, `input`, `reasoning`, and `_reasoning` metadata, unwraps
`action_input = dict(raw["input"])` once, and requires that mapping to be empty.
Action-only, flat, non-mapping, and non-empty calls are malformed; no legacy
flat route exists. Dispatch still uses the historical unknown-action message,
then the outer result boundary attaches the exact fresh `current_setting`
diagnostic to success, manual-degraded, malformed, and unknown results.

The no-op placeholder is `<agent>/settings/mcp.json` with exact valid v1 content
`{"schema_version": 1}`. Missing is normal; valid files are hot-reread on each
call. Invalid placeholder data yields a bounded `settings_error` diagnostic but
never changes registry validation, reconciliation, prompt XML, or manual
semantics. The settings diagnostic is evidence only, not MCP configuration.
