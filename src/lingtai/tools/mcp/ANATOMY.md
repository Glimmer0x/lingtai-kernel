...
- `mcp/plugin.json` — the Agent Plugins v1.0.0 manifest that makes this directory a real plugin package: `$schema` + `name: mcp`, plus the `ai.lingtai.tool` client-extension namespace naming the shipping Python package and the owned manual skill. There is deliberately **no** `mcp.json` beside it; pluginizing the tool must never become a route into `mcp_registry.jsonl`.
- `mcp/plugin.py` — the `MCP_TOOL_PLUGIN` descriptor (`lingtai.tools._plugin.BuiltinToolPlugin`). Binds the manifest, the Python package, and the owned `mcp-manual` skill, and raises at import when any of the three disagrees. `MCP_DECLARED_ACTIONS` is `("info",)`, with the reserved `manual` appended by the plugin, never declared here; `MCP_ACTIONS` is the plugin-composed public list.
- `mcp/skills/mcp-manual/` — the plugin's owned Agent Skill: skill documentation (`SKILL.md`) plus reference docs (`curated-addons.md`, `third-party-and-legacy.md`, `troubleshooting.md`) and scripts (`find_readme.py`). Discovered by `services.plugin_registry.read_plugin` like any plugin skill and mounted by `Agent._install_intrinsic_manuals` at `.library/intrinsic/capabilities/mcp/`, which is the copy the reserved `manual` action serves.
...
- **Plugin packaging grants no new power:** `src/lingtai/tools/mcp/` is a real Agent Plugins v1.0.0 package (`plugin.json` + `skills/mcp-manual/`), read by `services.plugin_registry.read_plugin` — the same reader third-party plugins go through — and mounted by `Agent._install_intrinsic_manuals`. The package ships **no** `mcp.json`: mounting it writes no `mcp_registry.jsonl` record (in particular no `source="plugin:mcp"` record), spawns nothing, and does not make `mcp` a declared or discovered Agent Plugin. Registration stays with `register_plugins` for operator-declared plugins; activation stays an explicit `init.json` top-level `mcp` entry. A tool package that did carry an `mcp.json` is reported as a packaging problem by `tools/_plugin.discover_tool_plugin` and its servers stay unregistered.
- **Manifest and descriptor cannot drift:** `MCP_TOOL_PLUGIN` re-states the manifest's name, shipping package, and owned manual skill, and `BuiltinToolPlugin` raises `BuiltinToolPluginError` at import when any of the three disagrees with `plugin.json`. The public surface is unchanged by the packaging: the action enum stays `["info", "manual"]`, `manual` still returns the flat `mcp_manual` shape, and `manual_path` still points at `.library/intrinsic/capabilities/mcp/SKILL.md`.
...
- **Manual:** `mcp/skills/mcp-manual/SKILL.md` — registration contract and usage guide, owned by the plugin package and mounted at `.library/intrinsic/capabilities/mcp/SKILL.md`.
