...
def _rerender_unread_digest(agent) -> str | None:
    ...
    publish_notification(
        ...
        instructions=(
            ...
            "email(action='read', input={...}) merely to see ordinary message content. After you "
            "handle a mail, prefer email(action='dismiss', input={'email_id': [id1, id2, ...]}) "
            "to mark it read and clear the "
            "notification without re-fetching content. Use email(action='read', input={...}) when "
            "you need to refresh source-of-truth mailbox state, inspect "
            "attachments/metadata, or intentionally fetch the producer record; "
            "use email(action='reply', input={...}) or email(action='reply_all', input={...}) "
            "to answer. Read and dismiss both accept "
            ...
            "if read/dismiss returns not_found, call email(action='check', input={"
