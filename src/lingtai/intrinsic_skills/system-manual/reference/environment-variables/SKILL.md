...
| `LINGTAI_SYSTEM_PROMPT_PRESSURE_RATIO` | Rendered system-prompt pressure warning threshold; default `0.4`. | Finite float strictly between `0` and `1`, process-wide for main-agent and daemon metadata snapshots. | Read on every metadata snapshot through the shared renderer, so a valid runtime environment change applies to the next snapshot. Missing/blank, non-numeric, non-finite, zero/negative, and `>=1` values fall back to `0.4`. | `src/lingtai/kernel/config.py`, `src/lingtai/kernel/meta_block.py`; affects bounded advisory metadata only and never authorizes access or exposes prompt body text. |
