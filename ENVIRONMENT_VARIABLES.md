...
| `LINGTAI_CACHE_MISS_BUDGET` | unset; valid `settings/system.json`, else `2_000_000` | Positive integer string; one agent/process | Highest-precedence source for the soft since-last-molt cache-miss budget guard | Every metadata snapshot; direct process changes need no restart | Invalid values fall through to valid System JSON, then `2_000_000`; the positive default cannot be disabled | `src/lingtai/tools/system/settings.py` via `Agent.resolve_cache_miss_budget()` | Soft steering only; never blocks, resets counters, or grants authority; no upper bound |
