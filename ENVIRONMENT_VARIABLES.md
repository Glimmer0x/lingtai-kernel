...
| `LINGTAI_CACHE_MISS_BUDGET` | unset; `1_000_000` default from `manifest.cache_miss_budget` | Positive integer; one agent/process | Soft since-last-molt cache-miss budget guard | Each cache-miss budget resolution (meta snapshots); no restart | Missing, non-int, bool, zero, or negative values fall back to the configured/default budget | `src/lingtai/kernel/meta_block.py` | Soft molt steering only; never blocks; not an authorization boundary |
