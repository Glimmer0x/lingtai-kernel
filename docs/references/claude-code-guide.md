...
`knowledge`, `skills`, `shell`, `avatar`, `daemon`, `mcp`, `plugin`, `task_card`, and `file` boot by default on every Agent (`CORE_DEFAULTS`); `vision` and `web` require provider config and stay explicit opt-in.
