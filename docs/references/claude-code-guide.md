...
| **Capabilities** | Composable capabilities (file [unified read/write/edit/glob/grep], knowledge, skills, shell, avatar, daemon, mcp, task_card, vision, web) | Declared at construction via `capabilities=` on Agent |
...
| `shell` | `capabilities={"shell": {"policy_file": "p.json"}}` or `{"shell": {"yolo": True}}` | Shell command execution with the active dialect. Defaults to `{"yolo": True}` |
...
| `vision` | `capabilities=["vision"]` or `{"vision": {"vision_service": svc}}` | Image understanding (LLM multimodal or dedicated VisionService). Explicit opt-in — not in the core default floor. |
...
`knowledge`, `skills`, `shell`, `avatar`, `daemon`, `mcp`, `task_card`, and `file` boot by default on every Agent (`CORE_DEFAULTS`); `vision` and `web` require provider config and stay explicit opt-in.
