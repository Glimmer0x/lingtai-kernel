...
def _resp(text="", tool_calls=None, *, raw=None, usage=None):
    return LLMResponse(
        text=text,
...
def _completion_required_loop_fixture(
    tmp_path,
    monkeypatch,
    responses,
    *,
    ...
    """Build the smallest in-process loop fixture with daemon_common required."""
    agent = _make_agent(tmp_path, ["daemon"])
    service = _CanonicalFakeService([list(responses)])
    import lingtai.llm.service as service_mod
    ...
    run_dir = _make_run_dir(
        agent,
        em_id="em-test",
        task="completion recovery test",
        tools=tools,
        max_turns=max_turns,
    )
    completion_path = run_dir.path / "daemon_completion.json"
    common = {"name": "daemon_common", "transport": "stdio"}
    ...
    base_schemas, base_dispatch = mgr._build_tool_surface(list(tools))
    finish_schema = FunctionSchema(
        name="finish",
        description="record the daemon completion",
    ...
    def finish(args):
        ...
        for key in ("summary", "reason", "artifacts"):
            if key in args:
                payload[key] = args[key]
        completion_path.write_text(json.dumps(payload), encoding="utf-8")
        return {"status": "ok", "completion_status": payload["status"]}

    schemas = [*base_schemas, finish_schema]
    dispatch = {**base_dispatch, "finish": finish}
    mgr._emanations["em-test"] = {
        "followup_buffer": "",
        "followup_lock": threading.Lock(),
        "run_dir": run_dir,
    }
    return agent, mgr, service, run_dir, schemas, dispatch, completion_path


def _daemon_events(run_dir):
...
def _recovery_events(run_dir):
    return [
        event
        for event in _daemon_events(run_dir)
        if event["event"] == "daemon_completion_recovery_decision"
    ]


def _run_completion(fixture, *, cancel_event=None, timeout_event=None, max_turns=4):
    _, mgr, _, run_dir, schemas, dispatch, _ = fixture
...
def test_semantic_empty_completion_recovery_can_finish_done(
    tmp_path, monkeypatch, blank_text
):
    finish = ToolCall(name="finish", args={"status": "done"}, id="finish-recovery")
    fixture = _completion_required_loop_fixture(
        tmp_path,
        monkeypatch,
        [_resp(blank_text), _resp(tool_calls=[finish]), _resp("recovered result")],
    )

    assert _run_completion(fixture) == "recovered result"
    _, _, service, run_dir, _, _, completion_path = fixture
    assert json.loads(completion_path.read_text())["status"] == "done"
    assert service.sessions[0].sent_messages[1] == daemon_tool.DAEMON_COMPLETION_RECOVERY_PROMPT
    events = _recovery_events(run_dir)
    assert len(events) == 1
    assert events[0]["text_semantically_empty"] is True
    assert events[0]["text_char_count"] == (len(blank_text) if isinstance(blank_text, str) else 0)
    assert events[0]["tool_call_count"] == 0
    assert events[0]["tool_names"] == []


def test_semantic_empty_recovery_is_one_shot_and_keeps_failure_without_finish(
    tmp_path, monkeypatch
):
    fixture = _completion_required_loop_fixture(
        tmp_path, monkeypatch, [_resp(""), _resp(" \t\n")]
    )

    with pytest.raises(RuntimeError, match="completion MCP contract"):
        _run_completion(fixture)
    _, _, service, run_dir, _, _, completion_path = fixture
    assert not completion_path.exists()
    assert len(service.sessions[0].sent_messages) == 2
    assert len(_recovery_events(run_dir)) == 1


@pytest.mark.parametrize("preexisting", [True, False])
def test_nonempty_final_and_valid_finish_do_not_recover(
    tmp_path, monkeypatch, preexisting
):
    finish = ToolCall(name="finish", args={"status": "done"}, id="finish-current")
    responses = (
        [_resp("ordinary final")]
        if preexisting
        else [_resp(tool_calls=[finish]), _resp("current finish final")]
    )
    fixture = _completion_required_loop_fixture(tmp_path, monkeypatch, responses)
    _, _, service, run_dir, _, _, completion_path = fixture
    if preexisting:
        completion_path.write_text(json.dumps({"status": "done", "run_id": run_dir.run_id}))

    expected = "ordinary final" if preexisting else "current finish final"
    assert _run_completion(fixture) == expected
    assert len(service.sessions[0].sent_messages) == (1 if preexisting else 2)
    assert not _recovery_events(run_dir)


@pytest.mark.parametrize("existing", ["failed", "incomplete", "malformed"])
def test_existing_non_done_or_malformed_completion_never_recovers(
    tmp_path, monkeypatch, existing
):
    fixture = _completion_required_loop_fixture(tmp_path, monkeypatch, [_resp(" \t")])
    _, _, service, run_dir, _, _, completion_path = fixture
    if existing == "malformed":
        completion_path.write_text("{not-json")
    else:
        payload = {"status": existing, "run_id": run_dir.run_id, "reason": "test reason"}
        completion_path.write_text(json.dumps(payload))
...
def test_semantic_empty_recovery_fails_closed_when_malformed_receipt_races_into_read(
    tmp_path, monkeypatch
):
    fixture = _completion_required_loop_fixture(
        tmp_path, monkeypatch, [_resp(""), _resp("should not be sent")]
    )
    _, _, service, run_dir, _, _, completion_path = fixture
    assert not completion_path.exists()
    original_read_text = Path.read_text
    raced_into_read = False

    def read_text_after_malformed_receipt_race(path, *args, **kwargs):
        nonlocal raced_into_read
        if path == completion_path and not raced_into_read:
            raced_into_read = True
            with pytest.raises(FileNotFoundError):
                path.stat()
            path.write_text("{not-json", encoding="utf-8")
        return original_read_text(path, *args, **kwargs)

    monkeypatch.setattr(Path, "read_text", read_text_after_malformed_receipt_race)

    with pytest.raises(RuntimeError, match="completion MCP contract"):
        _run_completion(fixture)
    assert raced_into_read is True
    assert completion_path.read_text(encoding="utf-8") == "{not-json"
    assert len(service.sessions[0].sent_messages) == 1
    assert not _recovery_events(run_dir)


def test_semantic_empty_recovery_provider_exception_stays_failed(tmp_path, monkeypatch):
    fixture = _completion_required_loop_fixture(
        tmp_path, monkeypatch, [_resp(""), RuntimeError("provider boom")]
    )

    with pytest.raises(RuntimeError, match="provider boom"):
        _run_completion(fixture)
    _, _, _, run_dir, _, _, _ = fixture
    assert json.loads(run_dir.daemon_json_path.read_text())["state"] == "failed"
    assert len(_recovery_events(run_dir)) == 1


@pytest.mark.parametrize("timeout", [False, True])
def test_semantic_empty_recovery_respects_cancel_and_timeout(
    ...
    assert _run_completion(
        fixture, cancel_event=cancel, timeout_event=timeout_event
    ) == "[cancelled]"
    assert len(service.sessions[0].sent_messages) == 1
    assert not _recovery_events(run_dir)


@pytest.mark.parametrize("max_turns, expected_recovery", [(1, False), (2, True)])
def test_semantic_empty_recovery_requires_max_turn_headroom(
    tmp_path, monkeypatch, max_turns, expected_recovery
):
    finish = ToolCall(name="finish", args={"status": "done"}, id="finish-headroom")
    fixture = _completion_required_loop_fixture(
        tmp_path,
        monkeypatch,
        [_resp(""), _resp(tool_calls=[finish]), _resp("done")],
        max_turns=max_turns,
    )
    _, _, _, run_dir, _, _, completion_path = fixture
    if expected_recovery:
        assert _run_completion(fixture, max_turns=max_turns) == "done"
        assert completion_path.exists()
    else:
        with pytest.raises(RuntimeError, match="completion MCP contract"):
            _run_completion(fixture, max_turns=max_turns)
        assert not completion_path.exists()
    assert len(_recovery_events(run_dir)) == int(expected_recovery)


def test_recovery_does_not_replay_a_preceding_side_effect_tool_call(
    tmp_path, monkeypatch
):
    calls = []
    side_effect = ToolCall(name="side_effect", args={"secret": "value"}, id="side-effect-1")
    finish = ToolCall(name="finish", args={"status": "done"}, id="finish-side-effect")
    fixture = _completion_required_loop_fixture(
        tmp_path,
        monkeypatch,
        [
            _resp(tool_calls=[side_effect]),
            _resp(""),
            _resp(tool_calls=[finish]),
            _resp("side effect recovered"),
        ],
    )
    _, _, service, _, schemas, dispatch, _ = fixture
    schemas.append(FunctionSchema(name="side_effect", description="probe", parameters={"type": "object", "properties": {}}))
    dispatch["side_effect"] = lambda args: calls.append(args) or {"ok": True}

    assert _run_completion(fixture) == "side effect recovered"
    assert calls == [{"secret": "value"}]
    assert [call[0] for call in service.make_tool_result_calls].count("side_effect") == 1


def test_recovery_shape_receipt_excludes_raw_provider_and_tool_payloads(
    tmp_path, monkeypatch
):
    secrets = {
        "text": "raw provider secret text",
        "argument": "raw tool argument secret",
        "stop_reason": "SECRET_STOP_SHOULD_NOT_BE_DURABLE",
        "block_kind": "SECRET_KIND_SHOULD_NOT_BE_DURABLE",
    }
    response = _resp(
        " \t",
        raw={
            "stop_reason": secrets["stop_reason"],
            "output": [{"type": secrets["block_kind"]}],
            "text": secrets["text"],
        },
        usage=UsageMetadata(
            input_tokens=12,
            ...
            thinking_tokens=1,
            cached_tokens=2,
            extra={"secret": secrets["argument"]},
        ),
    )
    fixture = _completion_required_loop_fixture(
        tmp_path, monkeypatch, [response, _resp("still text")]
    )
