...
state slice bound only to a narrow port, and ``daemon`` demonstrates a
manager-owning slice that consumes its current-agent
model/tool/preset/notification semantics through the capability-native runtime
port. All are mounted only through the registrar's controlled host path.
"""
